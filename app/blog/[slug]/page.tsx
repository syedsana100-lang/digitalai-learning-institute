...
import {
  ...
  calculateReadTime, getTableOfContents, resolveBlogImageUrl,
} from '@/lib/blog-data';
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  const ogImage = post.seo.ogImage || resolveBlogImageUrl(post.featuredImage, 1200, 630);
  return {
...
export default async function BlogPostPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const author = authors.find((a) => a.id === post.authorId) || getAuthorById(post.authorId);
  ...
  const articleSchema = {
    ...
    headline: post.title,
    description: post.seo.description,
    image: resolveBlogImageUrl(post.featuredImage, 1200, 630),
    ...
    <div className="pb-20">
      ...
      <div className="mx-auto grid max-w-6xl gap-10 px-5 py-8 lg:grid-cols-[1fr,260px] lg:px-8">
        <article>
          ...
          <img
            src={resolveBlogImageUrl(post.featuredImage, 900, 500)}
            alt={post.title}
            loading="lazy"
