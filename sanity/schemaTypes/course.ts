...
export default defineType({
  ...
  fields: [
    ...
    defineField({ name: 'whyLearn', title: 'Why Learn This Course', type: 'text', rows: 4 }),
    defineField({ name: 'careerOpportunities', title: 'Career Opportunities (job roles)', type: 'array', of: [{ type: 'string' }] }),
    defineField({ name: 'industryDemand', title: 'Industry Demand', type: 'text', rows: 3, description: 'Directional framing only — never invent numeric stats.' }),
    defineField({ name: 'careerRoadmap', title: 'Career Roadmap', type: 'array', of: [{ type: 'string' }] }),
    defineField({
      name: 'brochure',
      title: 'Course Brochure PDF',
      type: 'file',
      options: { accept: 'application/pdf' },
      description: 'Optional. When uploaded, a "Download Course Brochure" button appears on the course page.',
    }),
